...
for d in ${dirs[@]}; do
    for t in ${targets[@]}; do
        if [[ ! -f "${outdir}/${d}/${t}.txt" ]]; then
            ...
            command="python ../source/ae_other.py --data_dir ../data/${d} --name ${t} --pretrain_epochs 75 --dims_layers_ae 7000 4000 500 1000 100 --batch_size 50 --lr 0.0001 --save_dir ${outdir}/${d} --use_dropout --class_weight"
