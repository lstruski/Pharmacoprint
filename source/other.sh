...
for d in ${dirs[@]}; do
    for t in ${targets[@]}; do
        ...
        if [[ ! -f "${outdir}/${d}/${t}.txt" ]]; then
            nice -9 taskset -c 15-45 python other.py --data_dir ./data/${d} --name ${t} --class_weight --logdir ${outdir}/${d}
        fi

        if [[ ! -f "${outdir}/${d}/nn/${t}.txt" ]]; then
            command="nice -9 taskset -c 15-45 python supervised_ae.py --data_dir ./data/${d} --name ${t} --epochs 100 --dims_layers_classifier -1 500 100 50 10 --batch_size 50 --lr 0.0001 --save_dir ${outdir}/${d}/nn --use_dropout --procedure training_classifier --scale_loss 1"
