        ...
        for d in ${dirs[@]}; do
            for idt in ${!targets[@]}; do
                ...
                if [[ ! -d "${outdir}/${add2output}-target_${targets[${idt}]}/${d}" ]]; then
                    ...
                    command="python supervised_ae_multi_classifiers.py --data_dir ./data/${d} --name ${targets[${idt}]} --epochs 150 --dims_layers_ae ${ae_model} --dims_layers_classifier ${clr_model//-/ } --batch_size 50 --lr ${lr} --save_dir ${outdir}/${add2output}-target_${targets[${idt}]}/${d}_res --use_dropout --procedure training_classifier --scale 1 --ae ${outdir}/${add2output}-target_${targets[${idt}]}/${d}/ae_model.pth"
